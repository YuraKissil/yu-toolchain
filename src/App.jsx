...
const repoPath = "facebook/react"; // mdn/content

function renderGraph(root, data) {
  const layout = {
    title: `Commits to the ${repoPath} repo (${new Date(data[0].week * 1000).toLocaleDateString()} - ${new Date(data[data.length - 1].week * 1000).toLocaleDateString()})`,
    xaxis: {
      title: "Week",
      showgrid: true,
      zeroline: false,
...
function renderGrid(root, data) {
  const layout = {
    title: `Commits to the ${repoPath} repo, by day`,
    height: 350,
  };
...
function Example() {
  const { isPending, error, data } = useQuery({
    ...
    queryFn: () =>
      fetch(
        `https://api.github.com/repos/${repoPath}/stats/commit_activity`,
      ).then((res) => res.json()),
  });
  ...
  return (
    <main className={styles.container}>
      <h1 className={styles.title}>
        MDN Content repo commit activity - {repoPath}
      </h1>
      <div
        ref={(root) => {
          if (!root) return;
